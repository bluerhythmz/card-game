...
class Duelist {
  constructor(name, _cards, avatar, chosenCard) {
    this.name = name;
    this.cards = _cards;
    this.avatar = avatar;
...
function duelStart(duelist1, duelist2) {
...
function calculation(duelist1, duelist2) {
  let attackDifference = Math.abs(
    duelist1.chosenCard.attackPoints - duelist2.chosenCard.attackPoints
  );
  if (duelist1.chosenCard.attackPoints > duelist2.chosenCard.attackPoints) {
    ...
    counterDecrementer(duelist1, duelist2);

    setTimeout(() => {
      checkForWin(duelist1, duelist2);
    }, 1000);
  }
  if (duelist2.chosenCard.attackPoints > duelist1.chosenCard.attackPoints) {
    ...
    counterDecrementer2(duelist1, duelist2);
...
const checkForWin = (duelist1, duelist2) => {
  if (duelist2.lifePoints <= 0) {
    duelist1Wins(duelist1, duelist2);
  } else if (duelist1.lifePoints <= 0) {
    duelist2Wins(duelist1, duelist2);
  } else {
    setTimeout(() => {
      duelStart(duelist1, duelist2);
    }, 1800);
  }
};

const duelist1Wins = (duelist1, duelist2) => {
  field.playerTwoField.lifePointsElement.innerHTML = `${duelist2.name}: ${duelist2.lifePoints}`;
...
const duelist2Wins = (duelist1, duelist2) => {
  field.playerOneField.lifePointsElement.innerHTML = `${duelist1.name}: ${duelist1.lifePoints}`;
